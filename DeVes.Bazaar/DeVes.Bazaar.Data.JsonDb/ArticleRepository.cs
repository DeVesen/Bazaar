    ...
    public class ArticleRepository : BaseRepository<ArticleModel>, IArticleRepository
    {
                                                  ...
                                                  bool? isSold = null, bool? isReturned = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(title) is false)
                result = result.Where(p => p.Title != null && p.Title.ToLower().Contains(title.ToLower(), StringComparison.OrdinalIgnoreCase));
            if (string.IsNullOrWhiteSpace(category) is false)
                result = result.Where(p => p.Category != null && p.Category.ToLower().Contains(category.ToLower(), StringComparison.OrdinalIgnoreCase));
            if (string.IsNullOrWhiteSpace(manufacturer) is false)
                result = result.Where(p => p.Manufacturer != null && p.Manufacturer.ToLower().Contains(manufacturer.ToLower(), StringComparison.OrdinalIgnoreCase));
