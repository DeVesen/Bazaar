    ...
    public class SellerRepository : BaseRepository<SellerModel>, ISellerRepository
    {
                                                 ...
                                                 string zip, string town, string eMail)
        {
            ...
            if (string.IsNullOrWhiteSpace(firstName) is false)
                result = result.Where(p => p.FirstName != null && p.FirstName.ToLower().Contains(firstName.ToLower(), StringComparison.OrdinalIgnoreCase));
            if (string.IsNullOrWhiteSpace(lastName) is false)
                result = result.Where(p => p.LastName != null && p.LastName.ToLower().Contains(lastName.ToLower(), StringComparison.OrdinalIgnoreCase));

            if (string.IsNullOrWhiteSpace(zip) is false)
                result = result.Where(p => p.Zip != null && p.Zip.ToLower().Contains(zip.ToLower(), StringComparison.OrdinalIgnoreCase));
            if (string.IsNullOrWhiteSpace(town) is false)
                result = result.Where(p => p.Town != null && p.Town.ToLower().Contains(town.ToLower(), StringComparison.OrdinalIgnoreCase));
            if (string.IsNullOrWhiteSpace(eMail) is false)
                result = result.Where(p => p.EMail != null && p.EMail.ToLower().Contains(eMail.ToLower(), StringComparison.OrdinalIgnoreCase));
